...
def local_covid_cases() -> None:
    ...
    engine3.say("On " + str(date) + " there are " + str(new_cases) + " new cases of covid in " + location["my-city"])
    engine3.runAndWait()
    # Log alarm
    logging.info("Local covid cases were announced at: " + time.strftime("%H:%M", time.localtime()))
...
def national_covid_cases() -> str:
    """Uses Uk Gov. Api to return a string of the number of latest national covid_alarm_clock-19 cases.
        ...
        If covid Api response isn't 200: Returns error message
    """
...
def covid_rate(notif_list: list) -> None:
    ...
    if new_cases >= threshold:
        content = "The number of new covid cases in Exeter has passed " + str(threshold) + " and is now " + \
                  str(new_cases)
...
def schedule_event() -> None:
    ...
    if alarm_time:
        ...
        content = "Alarm set for the " + str(alarm_day) + "-" + str(alarm_month) + " at " + str(alarm_time) + \
                  " and contains daily covid update "
        if if_weather is None and if_news is not None:
            content = "Alarm set for the " + str(alarm_day) + "-" + str(alarm_month) + " at " + str(alarm_time) + \
                      " and contains daily covid update " + if_news
        if if_news is None and if_weather is not None:
            content = "Alarm set for the " + str(alarm_day) + "-" + str(alarm_month) + " at " + str(alarm_time) + \
                      " and contains daily covid update " + if_weather
        if if_news is not None and if_weather is not None:
            content = "Alarm set for the " + str(alarm_day) + "-" + str(alarm_month) + " at " + str(alarm_time) + \
                      " and contains daily covid update " + if_weather + " " + if_news
